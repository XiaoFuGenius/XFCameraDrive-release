//
//  XFCameraDrive.h
//  XFCameraDrive
//
//  Created by 胡文峰 on 2019/7/8.
//  Copyright © 2019 xiaofutech. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for XFCameraDrive.
FOUNDATION_EXPORT double XFCameraDriveVersionNumber;

//! Project version string for XFCameraDrive.
FOUNDATION_EXPORT const unsigned char XFCameraDriveVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <XFCameraDrive/PublicHeader.h>

/**
 XFCameraDrive版本号
<<<<<<< HEAD
 更新时间：2019.11.10 20:13
 */
#define XFCameraDrive_SDK_VERSION @"1.0.2"
=======
 更新时间：2019.11.18 13:28
 */
#define XFCameraDrive_SDK_VERSION @"1.0.3"
>>>>>>> master

#import "CTUVCHelper.h"


/**
XFCameraDrive Version History *
 
<<<<<<< HEAD
=======
 last update：2019.11.10 20:13   Version：1.0.2
>>>>>>> master
 last update：2019.11.07 16:05   Version：1.0.1
 last update：2019.09.20 12:35   Version：1.0.0
*/
